...
**应用** — 展示所有已安装应用、体积和上次打开时间。卸载前先列出完整计划：应用本体，
以及散落在各处的偏好设置、沙盒容器和缓存；每项都有体积，也可以单独保留。还包括开机
启动项。
...
**监控** — 实时 CPU（含每核心负载条）、二进制单位的内存、网络活动，以及支持按
CPU、GPU、内存排序的进程表。“实时排序”会随占用变化移动行，“固定位置”只刷新数值，
避免视线一直跳动。macOS 没有向普通 App 公开其他进程的实时 GPU 百分比，因此该列会
显示“—”，而不是用 CPU 或能耗伪造数据。离开页面后自动停止采样。
