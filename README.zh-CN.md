<div align="center">
  ...
  <img src="docs/screenshots/overview.jpg" alt="MacPleco 概览页：水位环显示 94.9 GB 可回收，实心的「开始清理」旁边是描边的「看看有哪些」，下方是磁盘、已安装应用、已运行时间和芯片四张等高卡片">
</div>
...
![清理页：已选体积以大号数字排在「移到废纸篓」按钮旁，下面是提示退出哪些应用可以多清理一些的横幅，再往下是每一类的人话说明与体积](docs/screenshots/clean.jpg)
...
![应用页：搜索框缩小了列表，筛选行里三个控件同高，每行显示体积、上次使用时间和一个描边的卸载按钮](docs/screenshots/apps.jpg)
...
![空间页：整幅矩阵树图铺满内容区，每个文件夹一种颜色，瓦片上直接写着体积与占比；上方是显示 478 GB 的面包屑，右上是文件夹、文件与其他项合计的图例](docs/screenshots/space.jpg)

**优化** — 针对八种具体毛病的小修复：“打开方式”菜单错乱、预览图空白、字体显示成
方块、DNS 过期、图标变白纸、共享盘里的 `.DS_Store`、窗口跑到屏幕外、访达卡死。
每一项都有常驻的“执行”按钮，也可以勾选多项一起执行。默认一个都不选。

![优化页：等高排列的修复项，每项写明它解决什么问题并自带一个「执行」按钮，默认都不勾选](docs/screenshots/tune-up.jpg)
...
![监控页滚动到 GPU 与网络两张仪表卡，各自带图例；下方是机器信息条与每核心负载，再往下是按 CPU 排序的可搜索进程表](docs/screenshots/monitor.jpg)

**菜单栏** — 菜单栏小鱼随时回答“磁盘还剩多少”，同时展示实时状态并一键进入清理。

**设置** — 语言、外观、菜单栏小鱼，以及完全磁盘访问权限的状态与一键前往授权。语言和
外观都即时生效，这里没有任何一项需要重启。

![MacPleco 设置窗口：语言与外观两个分段选择器、菜单栏小鱼开关、完全磁盘访问权限状态，以及写明 GPL-3.0 许可与灵感来源 Mole 的关于区](docs/screenshots/settings.jpg)
